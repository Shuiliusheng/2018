 ...
 exist(exist_)
{
	...
	//tag=物理地址宽度-索引位数-索引cacheline内部byte的位数+固定的tag位数
...
/**********************icache prefetch buffer，基本也和BP一致***************************/
	//预取缓存，tag是物理地址宽度加上固定宽度
	tag							   = XML->sys.physical_address_width + EXTRA_TAG_BITS;
	//数据宽度和指令cache中的cache line宽度一样
...
/**********************Instruction Buffer，基本也和BP一致***************************/
	...
	//假设IB是线程之间共享的时间片，每个获取操作将至少获取“获取宽度”指令
	//只有读写通用端口，没有单独的读/写端口
	interface_ip.num_rw_ports    = debug?1:XML->sys.core[ithCore].number_instruction_fetch_ports;
	interface_ip.num_rd_ports    = 0;
	interface_ip.num_wr_ports    = 0;
...
/***********************BTB基本的设置方法和BP中的设置方法一致***********************/
	if (coredynp.predictionW>0)
	{
		...
		//索引BTB需要的位数
		idx    					 	   = debug?9:int(ceil(log2(size/line/assoc)));
		//tag的大小：虚拟地址长度+线程数的区分位数+固定的tag位数
