...
double **V1;//[HIDDEN1][LEN];// the first argu
double **V2;//[HIDDEN2][HIDDEN1];// the second argu, setting like the formula
double **V3;//[OUT][HIDDEN2]
...
void train_init_memory()
{
	...
	V1=(double **)malloc(HIDDEN1*sizeof(double *));
	V2=(double **)malloc(HIDDEN2*sizeof(double *));
	V3=(double **)malloc(OUT*sizeof(double *));
	...
	for(int i=0;i<HIDDEN1;i++)
	{
		V1[i]=(double *)malloc(LEN*sizeof(double));
		for(int j=0;j<LEN;j++)
			V1[i][j]=0;
	}
	...
	for(int i=0;i<HIDDEN2;i++)
	{
		V2[i]=(double *)malloc(HIDDEN1*sizeof(double));
		for(int j=0;j<HIDDEN1;j++)
			V2[i][j]=0;
	}
	...
	for(int i=0;i<OUT;i++)
	{
		V3[i]=(double *)malloc(HIDDEN2*sizeof(double));
		for(int j=0;j<HIDDEN2;j++)
			V3[i][j]=0;
	}
