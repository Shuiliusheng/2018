...
bool init(char filename[])
{
    if(!read_image(filename))
        return false;
    cpu_dst=Mat(dW,dH,CV_8UC3,Scalar(0,0,0));
    gpu_dst=Mat(dW,dH,CV_8UC3,Scalar(0,0,0));
...
double cpu_rotate(double rad,unsigned char *src,unsigned char *dst)
{
	...
	unsigned long long cost_time =(tv1.tv_usec)-(tv.tv_usec);
...
{	
    ...
    for(int y=0;y<H;y++)
    {
        for(int x=0;x<W;x++)
        {
            int rx=(x-W/2)*cosrad+(y-H/2)*sinrad;
            int ry=(y-H/2)*cosrad-(x-W/2)*sinrad;
            rx=rx+dW/2;
            ry=ry+dH/2;
