...
double gpu_rotate(char filename[],unsigned char *src,unsigned char *dst,double rad,int W,int H,int dW,int dH)
{
	...
	unsigned char *src1=(unsigned char*)malloc(sizeof(char)*W*H*4);
	unsigned char *dst1=(unsigned char*)malloc(sizeof(char)*dW*dH*4);
	memset(dst1,255,dW*dH*4);
	for(int i=0;i<H;i++)
	{
		for(int j=0;j<W;j++)
		{
	...
	clImageDesc.image_height=H;
	clImageDesc.image_type=CL_MEM_OBJECT_IMAGE2D;
	memObjects[0]=clCreateImage(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,&clImageFormat, &clImageDesc, src1, &errNum);
	
	
	cl_image_format clImageFormat1;
	clImageFormat1.image_channel_order = CL_RGBA;
	clImageFormat1.image_channel_data_type = CL_UNSIGNED_INT8;
	
	cl_image_desc clImageDesc1;
	memset(&clImageDesc1,'\0',sizeof(cl_image_desc));
	clImageDesc1.image_width=dW;
	clImageDesc1.image_height=dH;
	clImageDesc1.image_type=CL_MEM_OBJECT_IMAGE2D;
	memObjects[1]=clCreateImage(context,CL_MEM_WRITE_ONLY,&clImageFormat1,&clImageDesc1,NULL,&errNum);
  ...
  // cout <<"create memory object successfully"<<endl;
	...
	size_t region[3]={dW,dH,1};
	status = clEnqueueReadImage(commandQueue, memObjects[1],CL_TRUE, origin,region,0,0,dst1,0,NULL, NULL);
